...
MTS_VARIANT void SamplingIntegrator<Float, Spectrum>::render_block(const Scene *scene,
    ...
    if constexpr (!is_array_v<Float>) {
        for (uint32_t i = 0; i < pixel_count && !should_stop(); ++i) {
            ...
            static bool adaptive_map_loaded = false;
            static bool adaptive_flag = false;
            static std::vector<std::vector<int>> adaptive_map;
            int width = sensor->film()->size().x();
            int height = sensor->film()->size().y();
            sampler->pixel = pos;
...
MTS_VARIANT void SamplingIntegrator<Float, Spectrum>::render_sample(
    ...
    ray.pixel_x = position_sample[0];
    ray.pixel_y = position_sample[1];

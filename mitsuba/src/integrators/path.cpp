...
template <typename Spectrum> float avg(Spectrum s) {
...
public:
    ...
    PathIntegrator(const Properties &props) : Base(props) {
        std::cout << "[MLOG] path integrator created" << std::endl;
    }
    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene, Sampler *sampler,
        ...
        static std::ofstream my_log("my_log.txt");
        ...
        std::vector<std::tuple<Point3f, Vector3f, float>> my_stack;
        ...
        for (int depth = 1;; ++depth) {
            ...
            if (any_or<true>(neq(emitter, nullptr)))
                if (emitter->is_environment() || !specular_camera_path)
                    result[active] +=
            ...
            if (likely(any_or<true>(active_e))) {
                ...
                // ! Print
                // std::cout << "Sample emitter " << si.p << " " << wo << " "
                //           << avg(bsdf_val * emitter_val) << stdp::endl;
                // float tmp_val = avg(bsdf_val * emitter_val);
                // if (my_stack.size() == 1 && avg(bsdf_val) > 0.0003) {
                    
                //     for (int i = my_stack.size() - 1; i >= 0; i--) {
                //         auto [position, direction, stack_bsdf_val] = my_stack[i];
                //         if (tmp_val >= 1e-4 && avg(bsdf_val) > 0.00001 && position[1] < 0.1&& direction.y() > 0) {
                //             static std::mutex mu;
                //             mu.lock();
                //             direction = normalize(direction);
                //             my_log << position.x() << " " << position.y() << " "
            ...
            auto world_pos = si.p;
            auto world_wi  = si.to_world(si.wi);
            auto world_wo  = si.to_world(bs.wo);

            // std::cout << "[MLog] " << world_pos << " " << world_wo << " "
            //<< bsdf_val[0] << std::endl;
            ...
            if (any_or<true>(neq(emitter, nullptr))) {
                ...
                //hit emitter
                /*float tmp_val = avg(bsdf_val * emitter->eval(si, active));
                 if (my_stack.size() == 1) {
                    for (int i = my_stack.size() - 1; i >= 0; i--) {
                        auto [position, direction, bsdf_val] = my_stack[i];
                        if (tmp_val >= 0 && position[1] < 0.1) {
                            static std::mutex mu;
                            mu.lock();
                            direction = normalize(direction);
                            my_log << position.x() << " " << position.y() << " "
            ...
            // ! Print
            // std::cout << "Sample bsdf " << world_pos << " " << world_wo << "
            // "
            //           << avg(bsdf_val) << std::endl;
            // my_stack.push_back({ world_pos, world_wo, avg(bsdf_val) });
        }
